...
const API_HOST = 'https://caiji.semaozy.net';
const API_URL = 'https://caiji.semaozy.net/inc/apijson_vod.php/provide/vod';
...
const logInfo = (message, data = null) => {
    ...
    OmniBox.log("info", `[色猫资源-DEBUG] ${output}`);
};

const logError = (message, error) => {
    OmniBox.log("error", `[色猫资源-DEBUG] ${message}: ${error.message || error}`);
};
...
function preprocessTitle(title) {
    if (!title) return '';
    const str = String(title);  // 确保转为字符串，防止 title 为数组等非字符串类型
    return str
